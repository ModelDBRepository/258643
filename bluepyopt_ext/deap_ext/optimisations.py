...
class WeightedSumFitnessWFeat(deap.base.Fitness):
    """Fitness that compares by weighted sum, and saves feature values"""

    def __init__(self, values=(), feature_values=(), obj_size=None):
        self.weights = [-1.0] * obj_size if obj_size is not None else [-1]

        super(WeightedSumFitnessWFeat, self).__init__(values)
...
class WSWFListIndividual(list):

    """Individual consisting of list with weighted sum and feature values fields"""

    def __init__(self, *args, **kwargs):
        """Constructor"""
        self.fitness = WeightedSumFitnessWFeat(obj_size=kwargs['obj_size'])
        del kwargs['obj_size']
        super(WSWFListIndividual, self).__init__(*args, **kwargs)
